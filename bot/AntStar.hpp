...
struct StarAnt {
	...
	float f; // f(n) = g(n) + h(n), på vägen mot målet bör denna variabel vara identisk hela vägen (iom att g blir större och f blir mindre)
	float g; // g(n) antal steg sedan start (enkel räknare)
	float h; // h(n) hierustisk beräkning för nod
	Pos p;
